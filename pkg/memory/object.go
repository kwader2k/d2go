	...
	for i := 0; i < 128; i++ {
		...
		for entranceUnitPtr > 0 {
			...
			if entranceType := gd.Process.ReadUInt(entranceUnitPtr+0x00, Uint32); entranceType == 5 {
				...
				// Read entrance mode to determine if it's selectable
				entranceMode := mode.ObjectMode(gd.Process.ReadUInt(entranceUnitPtr+0x0c, Uint32))
				...
				entrances = append(entrances, data.Entrance{
					...
					Selectable: entranceMode == mode.ObjectModeIdle,
					Position: data.Position{
						X: int(posX),
						Y: int(posY),
					},
